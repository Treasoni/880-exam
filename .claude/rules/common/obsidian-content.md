...
拼卷时随卷子生成，用户做完后对照答案在状态列写 `x`，判分脚本 `grade.py --sheet` 读取勾选。
...
1. `# 判分卡 · 卷子-01`
2. `> [!info] 判分说明`（对照答案在对应状态列写 `x`；没做的题留空）
3. `## 一、选择题` / `## 二、填空题` / `## 三、解答题`，每节一张 Markdown 表格：
...
```
   ...
   - 勾选标记：`[x]` / `[X]` / `✅` / `☑` 等；每行只勾一个状态，多勾会判分报错。
   - 答案列：选择/填空写答案；解答题写 `（见答案卷）`；解析册缺失写 `（解析册未找到）`。

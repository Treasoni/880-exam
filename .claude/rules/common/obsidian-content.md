...
3. `## 一、选择题` / `## 二、填空题` / `## 三、解答题`；每题以 `### 第 N 题` 起头，再给**题目原文 + 答案加粗 + 解析**，例如：
   ...
   **答案：** **4; $1-5e^{-2}$**
   ...
   #### 思路点拨        # 自定义解析的内部板块，渲染时降级为 ####
   ```

   - 题级标题统一写 `### 第 N 题`：答案卷动辄数十题、上千行，大纲（Outline）靠它按题跳转。题干行仍保留 `**N.**` 加粗题号——`lib880.split_answer_sheet` 靠它切块核验，也便于与判分表 `一1`/`二3` 对照，**不要**为了让标题与题干不重复而删掉题号。
   - 解析内部的 Markdown 标题（覆盖层自定义解析常用 `### 总体思路`）由渲染层降级：`lib880.demote_solution_headings(text, item_level=3)`，最浅标题落到 `####`，更深保持相对层级、封顶 `######`；源文本不改。**禁止**把题级 `###` 的解析直接拼进答案卷——那会在大纲里生成一串没有归属的平级条目（与错题本同一策略）。
   - 题目顺序、题号切块必须与 `papers.json` 记录一致，由 `scripts/lint_content.py` 的 `answer_sheet_key_errors` 校验。
4. `## 关联`：`- 对应卷子：[[卷子-01]]`

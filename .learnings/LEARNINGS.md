...
**摘要**：用户要求把详细讲解放入错题本时，必须修改 `question-index.json` 的题目解析并重跑生成脚本，而不是直接改错题本产物。

**详情**：
- 事实：本次先直接把两个讲解 callout 写入错题本，随后才补回题目索引并运行 `python3 scripts/wrong_book.py`。
- 根因：把用户当前查看的 Markdown 当成事实源，忽略错题本由索引和判分记录渲染。
- 下次做法：先定位 qid 和索引条目，更新 `solution`，重生成后 grep 验证目标题目包含新增内容。
...
**摘要**：处理 $n$ 次根式之差时，写成指数形式后可用指数函数中值定理拆出两个指数的差。

**详情**：
- 事实：$(n+1)^{1/n}-n^{1/(n+1)}=e^{A_n}-e^{B_n}$，其中 $A_n=\ln(n+1)/n$、$B_n=\ln n/(n+1)$。
- 根因：直接比较两个根式不直观，容易看不出差值的阶数。
- 下次做法：先用 $e^{A_n}-e^{B_n}=e^{\xi_n}(A_n-B_n)$，再分别处理 $A_n-B_n$ 与 $e^{1/n}-1$。

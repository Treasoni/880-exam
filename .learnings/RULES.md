...
- **重建保护用户状态**：`--rebuild` 会清空判分卡未判勾选、重置 frontmatter `date`、并按索引原文重渲染答案卷；重建前先提交/备份受影响卷目录，重建后逐项核验（判分卡勾选、其它题解析、`date`），发现被回退立即 `git checkout HEAD --` 还原。
...
- **生成前置校验**：`wrong_book.py` 写入前拒绝控制字符、独立式 `\[...\]` 与转义形式，并把行内 `\(...\)` 归一为 `$...$`；重建后必须运行 `python3 scripts/lint_content.py`。

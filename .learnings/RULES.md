...
- **重建入口 --rebuild**：重建已有卷产物用 `python3 scripts/make_paper.py --rebuild <paper_id>`（从 papers.json 读同一批题）；只重刷答案卷用 `--rebuild-answers <paper_id|all>`（纯投影：内容未变不写盘，`date` 保留、`updated` 只在真有改动时推进）。
- **重建保护用户输入**：`--rebuild` 在判分卡有勾选或该卷已有判分记录时**保留判分卡不重建**（打印「保留判分卡不重建」），卷子/答案仅在无判分记录时重刷；判据是 `lib880.card_ticked_lines`。确需重排判分卡（**会清空勾选**）才加 `--rebuild-card`，且它只清点自己打印的「原有 N 处勾选」。用 `--rebuild-card` 前先提交/备份受影响卷目录，重建后逐项核验（勾选、其它题解析、`date`），被回退立即 `git checkout HEAD --` 还原。**勾选数与其它卷面状态一律以重建前的工作区实测为准**（`grep -c '\[x\]'`），不要沿用记忆或上一轮的数字——用户可能在两次重建之间继续做题。
- **答案卷改动先写事实源**：答案卷任何解析改动必须先写入 `workspace/question-index.json` 的 `solution`，再 `--rebuild-answers`（或 `--rebuild`）重刷；禁止只改答案卷（否则下次重建即丢失）。
...
- **坏内容判据单一出处**：一类产物的校验正则/常量只放在 `scripts/lib880.py`，渲染层与 `lint_content.py` 都引用它；修「某种写法漏检」时把同族形式一次列全，禁止在多个文件各存一份正则。判分卡勾选标记同此：`TICK_MARKS` / `CHECK_RE` / `TICK_RE` / `card_ticked_lines` 只在 `lib880` 定义，`grade.py`（读勾选判分）与 `make_paper.py`（判断卡上是否已有用户输入）共用一份——口径一分叉，一边放宽标记、另一边就会把已勾选的卡当空白卡重建掉。
...
- **测试补丁要覆盖全部写盘目标**：用 `patch.multiple(lib880, …)` 隔离的测试，必须把**本次流程会写到的每个路径**（`PAPERS_DIR` / `PAPERS_PATH` / `ATTEMPTS_PATH` / `WRONG_BOOK_PATH` / `PROGRESS_PATH` / `SOLUTION_OVERRIDES_PATH`…）都指向 tmp，并在跑主流程前断言一遍；只挂读路径时测试会**静默打到真实 vault**——2026-09-24 那次 `--rebuild-answers` 真在 vault 上跑，只因「内容未变不写盘」才没落笔。测试文件头的「所有写入都限制在临时目录」是待验证的断言，不是既成事实。
...
- **复发缺陷修三层**：修一类反复复发的缺陷，机制要同时盖住①源头校验（坏数据在校验点暴露）、②陈旧检测（存量产物不静默留坏）、③流程步骤（skill 固定动作）；只补一层下次仍复发，三层都绿才归档经验库记录。**案**：判分卡勾选被 `--rebuild` 清空——2026-09-13 归档时只写了流程层（重建前备份 + 重建后核验），2026-09-24 照旧复发（paper-04 被清两次，代码注释留痕）；补齐①源头（勾选判据单点进 `lib880`，守卫与判分共用一份）②存量（重建时检测卡上勾选/已判分 → 保留并打印，留 `--rebuild-card` 逃生口）③流程（`880-paper` skill 写清三种重建入口的语义）＋回归测试（`tests/test_core.py` 锁死「有勾选的卡重建后逐字节不变」）后止住。

...
1. **查看状态清单**：
   ...
   # 线代：python3 scripts/wrong_book.py --subject linear-algebra --list-states
   ```
   或直接读高数 `workspace/wrong-book/错题本.md` / 线代 `workspace/wrong-book/线代错题本.md`。
2. **重生成错题本**：
   ```
   python3 scripts/wrong_book.py
   # 线代：python3 scripts/wrong_book.py --subject linear-algebra
   ```
3. **更新复习状态**：
   ```
   python3 scripts/wrong_book.py --mark gs-c01-basic-choice-003=已掌握
   # 线代：python3 scripts/wrong_book.py --subject linear-algebra --mark la-c07-basic-choice-001=已掌握
   ```
   状态可选：未复习 / 已重做 / 已掌握。
...
- 每题条目带来源卷子链接（高数 `[[卷子-XX]]`；线代 `[[线代卷子-XX]]`），取最近一次判分的卷子；

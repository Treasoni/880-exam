...
**5.** 设二阶常系数非齐次线性微分方程 $y'' + ay' + by = (cx + d)e^{2x}$ 有特解 $y = 2e^{x} + (x^{2} - 1)e^{2x}$ ，求该方程的通解，并求 a, b, c, d 的值.
...
**6.** 在 $x = 0$ 的右邻域内，用多项式 $\mathrm{e} + ax + bx^2$ 近似表示函数 $f(x) = (1 + x)^{\frac{1}{x}}$ ，使其误差是比 $x^2$ 高阶的无穷小 $(x \to 0^{+})$ ，求 $a, b$ 的值.
